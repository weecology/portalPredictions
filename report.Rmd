...
most_recent_date = dplyr::filter(newmoons_table,period==most_recent_period) %>% dplyr::select(censusdate) %>% pull(censusdate)

# get most recent forecast prior to data collection
files = data.frame(names = list.files(path = './predictions', pattern = '*Allforecasts.csv'))
