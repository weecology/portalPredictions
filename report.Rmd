...
observed = portalr::abundance(clean=FALSE, shape='flat', level='treatment', plots='longterm', time = 'newmoon') %>%
            dplyr::filter(newmoonnumber == max(newmoonnumber), treatment=='control')

# get new moon number associated with this period
most_recent_date = read.csv('PortalData/Rodents/moon_dates.csv') %>%
...
# get most recent rodent data
observed = portalr::abundance(clean=FALSE, shape='flat', time = 'newmoon') %>%
