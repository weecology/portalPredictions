# Naive1
#
# Model Naive 1 is a flexible exponential smoothing state space model fit
#  using the forecast and ets functions with with BoxCox.lambda(0)
#  and allow.multiplicative.trend = TRUE
  ...
  naive1 <- function(abundances, forecast_date, forecast_months, 
    ...
    # fit the ets model and forecast with it
      ...
      model01 <- forecast(interpolated_abundances, h = num_forecast_months,
                          level = CI_level, BoxCox.lambda(0),
...
# Write results out
  ...
  write.csv(forecasts, 
            file.path('tmp', 
                paste("naive1", filename_suffix, ".csv", sep = "")),
            row.names = FALSE)
  write.csv(forecast_aics, 
            file.path('tmp', 
                paste("naive1", filename_suffix, "_model_aic.csv", sep = "")),
            row.names = FALSE)
