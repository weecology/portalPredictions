...
fitted using the `auto.arima` (with the `lambda` argument set to 0) and 
`forecast` functions in the **forecast** package [@Hyndman2013; @Hyndman2017]
and the `forecast_autoarima` function in our **portalPredictions** package 
[@PortalPredictions].
...
ARIMA(p, q, q) [@Box1970]. While the `auto.arima` function allows for seasonal
...
(q), and are represented as GARCH(p, q) [@Liboschik2017a]. The PETS model is 
